...
public class ByteService {
	...
	public void fileByteOutput() {
		...
		try {
			...
			*/
			...
			System.out.println("[수행 시간] : " + (endTime - startTime) + "ns"); // 72500ns
			...
			if(fos != null) {
				try {
					// 스트림이 정상 생성된 경우
					// 스트림을 닫는다.
					// close() 메서드도 IOExceptioin 발생하기 때문에
					// try-catch 이용하여 예오 처리 필수.
					fos.close();
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	public void bufferedFileByteOutput() {
		...
		try {
			...
			*/
			...
			System.out.println("[수행 시간] : " + (endTime - startTime) + "ns"); // 35200ns
	 ...
	 * 
	 * InputStrea의 경우 외부 파일을 읽어올 때 사용
	...
	public void fileCopy() {
		...
		try {
			...
			// 입력용 스트림 생성
			fis = new FileInputStream(target); // 입력용 기반 입력용 스트림 생성
